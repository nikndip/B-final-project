...
func (api *API) Statistics(w http.ResponseWriter, r *http.Request) {
  ...
  weekly := buildWeeklyStats(api, userID)
  monthly := buildMonthlyTrends(api, userID)
  categories := buildCategoryStats(api, userID)
  records := buildRecordStats(api, userID)
  ...
  writeJSON(w, http.StatusOK, map[string]any{
    ...
    "monthly_points": strings.Join(points, " "),
  })
}

func buildWeeklyStats(api *API, userID string) []weeklyStat {
  now := time.Now()
...
func buildMonthlyTrends(api *API, userID string) []monthlyTrend {
  now := time.Now()
...
func buildCategoryStats(api *API, userID string) []categoryStat {
...
func buildRecordStats(api *API, userID string) []recordStat {
